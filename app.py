...
def save_chat_history(user_id, user_text, model_text):
    try:
        ...
        user_ref.set({'chat_history': current_history}, merge=True)

        # 2. 寫入永久日誌 (full_logs) - 給管理員用
        # 這裡使用 add() 會自動產生亂碼 ID，確保每一句都存下來
        user_ref.collection('full_logs').add({
            'user': user_text,
            'model': model_text,
...
def clear_chat_history(user_id):
    try:
        doc_ref = db.collection('users').document(user_id)
        # 只清空 chat_history，full_logs 不會動
        ...
        if response.parts:
            for part in response.parts:
                if part.function_call:
                    ...
                    func_response_parts.append({"function_response": {"name": fname, "response": {"result": "Success" if isinstance(api_result, dict) else str(api_result)}}})
