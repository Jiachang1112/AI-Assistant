...
def get_system_instruction():
    ...
    2. 使用者說的時間如果是相對時間，請根據現在時間轉換成 ISO 8601 格式。
...
@app.route("/")
def home():
    return "OK - Dynamic Quick Reply Bot", 200
...
def oauth2callback():
    if 'state' not in session:
        return "錯誤：瀏覽器 Session 失效。請嘗試複製連結到 Chrome 開啟。"
    ...
    try:
        ...
        if response.parts and response.parts[0].function_call:
            ...
            if isinstance(api_result, dict) and 'htmlLink' in api_result:
                ...
                flex_msg.quick_reply = get_quick_reply(user_id) # 帶入 user_id
                ...
                line_bot_api.reply_message(
                    ...
                    TextSendMessage(
                        ...
                        quick_reply=get_quick_reply(user_id) # 帶入 user_id
                    )
                )
        else:
            line_bot_api.reply_message(
        ...
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text="系統忙碌中，請稍後再試。", quick_reply=get_quick_reply(user_id)))
